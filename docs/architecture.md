...
For 微信云托管 / CloudBase Run, use `STORAGE_MODE=mysql` with `DATABASE_URL=mysql://<username>:<password>@<internal-host>:<port>/<database>`. The MySQL store keeps core product entities in normalized tables: `users`, `families`, `family_members`, `children`, `classes`, `lessons`, and `auth_credentials`. It migrates legacy core records from `kv_store` into empty normalized tables on startup, writes later core changes back to the normalized tables, and refreshes those tables at the beginning of each HTTP request before authentication and business logic run. Less central or fast-changing records remain in `kv_store`, including sessions, attendance side records, leaves, lesson change records, suspensions, reminder settings, and theme preferences. Cloud hosting containers should be treated as stateless; local file, SQLite, and memory modes are not production-safe when instances are recreated or scaled.
